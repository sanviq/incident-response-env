...
# Set to an integer to cap scenarios per task during development, e.g. 2
...
def log_start(task: str, env: str, model: str, scenario: int) -> None:
    print(
        f"[START] task={task} scenario={scenario} env={env} model={model}",
        flush=True,
    )
...
# ------------------------------------------------------------------
# Rule-based fallback  (used when the LLM call fails or key is unset)
# ------------------------------------------------------------------
def _extract_severity(alert: str) -> str:
    ...
    if any(
        k in a
        for k in [
            "99%", "98%", "97%", "503", "down", "failing", "data loss",
            "oom", "imminent", "outage", "exhausted", "not ready",
            "cannot log",
        ]
    ):
        return "critical"
    if any(
        k in a
        for k in [
            "76%", "78%", "84%", "85%", "88%", "trending", "climbing",
            "lag", "latency", "leak", "elevated", "slow",
            "failed silently", "cron",
        ]
    ):
        return "medium"
    return "low"
...
def _best_remediation_option(observation: Dict[str, Any]) -> str:
    ...
    if any(k in alert for k in ["lock", "blocking"]):
        for letter, text in options.items():
            if any(
                k in text.lower()
                for k in ["terminate", "pg_terminate", "kill"]
            ):
                return letter
...
def rule_based_response(observation: Dict[str, Any]) -> str:
    ...
    if task == "cascading-alerts" and pending:
        ...
        if sev == "critical" or any(
            k in atxt
            for k in ["503", "down", "failing", "outage", "not ready"]
        ):
            action = (
                "Immediately page the on-call engineer and restart "
                "the affected service to restore availability."
            )
        elif any(k in atxt for k in ["lag", "replica", "memory", "cache", "cron"]):
            action = (
                "Investigate root cause and apply targeted fix; "
...
def llm_response(client: OpenAI, observation: Dict[str, Any]) -> str:
    try:
        ...
        print(
            f"[DEBUG] LLM call failed ({type(exc).__name__}): {exc}",
            flush=True,
        )
...
def run_episode(
    ...
    try:
        ...
        # Per-step rewards are already emitted above; final score is the mean.
        # For cascading-alerts this captures partial credit across all 3 steps.
        ...
        print(
            f"[DEBUG] Episode error for task '{task_name}' "
            f"scenario {scenario_index}: {exc}",
            flush=True,
        )
...
# ------------------------------------------------------------------
# Task config — max_steps mirrors each task's episode length
# ------------------------------------------------------------------
TASK_CONFIG = [
...
def main() -> None:
    print(
        f"[DEBUG] Starting | API: {API_BASE_URL} | "
        f"Model: {MODEL_NAME} | Env: {ENV_BASE_URL}",
        flush=True,
    )
    ...
    for task_cfg in TASK_CONFIG:
        ...
        print(
            ...
            f"running {n_scenarios} scenario(s) × {max_steps} step(s)",
            flush=True,
        )
    ...
    for task_name, scores in task_scores.items():
        ...
        print(
            f"[DEBUG]   {task_name}: "
            f"avg={avg:.3f} | passed={wins}/{len(scores)} scenarios",
            flush=True,
        )
    ...
    print(
        f"[DEBUG] Overall average score: {overall_avg:.3f}",
        flush=True,
    )
